...
class ServerlessPlugin {
  ...
  async pushImageToEcr() {
    ...
    try {
      execSync(`$(aws ecr get-login --no-include-email --region ${this.options.region}) | docker login --username AWS --password-stdin ${config.Ecr.AwsAccountId}.dkr.ecr.${this.options.region}.amazonaws.com`);
      ...
      try {
        execSync(`$(aws ecr get-login-password --region ${this.options.region}) | docker login --username AWS --password-stdin ${config.Ecr.AwsAccountId}.dkr.ecr.${this.options.region}.amazonaws.com`);
        this.serverless.cli.log(`Successfully configured docker with ECR credentials`);
      } catch (err) {
